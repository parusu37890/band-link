...
## 2026-09-15 リリース判定の前にまだ塞いでいない2点（テスト計画のスコープ外）

`docs/test-plan/test-cases.md`の全ケース（ST-001..055、SEC-001..018、NFT-002..013）は実施済みだが、このテスト計画自体がカバーしていない、リリース前に一度も確認していない項目が2つ残っている。

1. **実SMTPでのメール送信が一度も確認されていない**: このプロジェクトの検証は最初から最後まで`MAIL_HOST`未設定の環境で行われており、`MailService`は毎回「verification mail not sent」と警告を出すだけで実際には送信していない。確認メール・パスワード再設定メールが実在のメールアドレスへ本当に届くか、リンクが実際に機能するかは未検証のまま
2. **本番相当環境への実デプロイが未リハーサル**: ローカルQA DBとCIのDockerコンテナ上でしか動作確認しておらず、実際のデプロイ手順・ロールバック手順を一度も通していない

実機ブラウザ（Safari、実iOS/Android）・実スクリーンリーダー・実LINE OAuth・ログ基盤（Elasticsearch/Kibana）の確認も引き続き未着手（詳細は下記の各節を参照）。次に着手する担当は、この4点（実機・実SMTP・実デプロイ・実LINE OAuth／ログ基盤）を埋めてからリリース判定を行うこと。
