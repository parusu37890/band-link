-- 修正前に退会したアカウントを、いまの退会処理と同じ状態に揃える。
--
...
--
-- 行そのものは消さない。requirements 3章が「相手側には送信済み会話を残し、送信者名を
-- 『退会済みユーザー』に置き換える」と定めており、会話の相手を指すidが要るため。
-- 残すのはidと退会済みという状態だけで、本人を特定できる情報は残さない。
