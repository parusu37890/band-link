...
$features=@(
 ...
 @{Code='POST';Req='R-POST-01..08';Name='Recruitment-post';Inputs=@('recruit','join','empty-body','body-boundary','missing-choice','too-many-choice','zero-images','five-images','six-images');Users=@('anonymous','unverified','user','suspended','withdrawn','admin','other-owner');Data=@('published','closed','expired','edit-locked','owner','non-owner');Ops=@('list','detail','create','edit','close','republish','delete','double-submit')},
 @{Code='SEARCH';Req='R-SRCH-01..05';Name='Search';Inputs=@('empty-keyword','title-hit','body-hit','area-hit','no-hit','max-length','huge-cursor','invalid-cursor');Users=@('anonymous','user','suspended','withdrawn','admin');Data=@('zero','one','many','tie','history','no-history');Ops=@('initial','search','filter','sort','load-more','retry','clear')},
