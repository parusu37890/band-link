 ...
 *
 * NFT-004: PostImageService.add/addAll have the exact same class of bug - countByPostId(postId) >= 4
 * is checked, then a row is inserted, with no atomicity between the two. Two concurrent uploads to
 * the same post can both pass the count check before either commits, producing 5+ images. A "count
 * must stay <= 4" rule cannot be expressed as a plain UNIQUE/CHECK constraint (CHECK cannot see other
 ...
 * committed sibling rows and rejects the insert once 4 already exist. PostImageService's own count
 * check stays as the fast path; its saveAndFlush catch both converts the loser's rejection into the
 * same RuleViolationException message the synchronous over-4 case gets, and deletes the file it had
