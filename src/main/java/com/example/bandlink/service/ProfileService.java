...
@Service public class ProfileService {
 ...
 @Transactional public User update(Long id, ProfileUpdateRequest r){ User u=users.findById(id).orElseThrow(()->new IllegalArgumentException("ユーザーが見つかりません")); if(r.username()!=null)u.setUsername(r.username().trim());u.setGender(r.gender());u.setBio(r.bio());u.setAge(r.age());u.setExperienceYears(r.experienceYears());u.setVideoUrl(r.videoUrl());if(r.partIds()!=null){u.getParts().clear();u.getParts().addAll(parts.findAllById(r.partIds()));}if(r.genreIds()!=null){u.getGenres().clear();u.getGenres().addAll(genres.findAllById(r.genreIds()));}if(r.stanceIds()!=null){u.getStances().clear();u.getStances().addAll(stances.findAllById(r.stanceIds()));}if(r.prefectureIds()!=null){if(r.prefectureIds().size()>3)throw new IllegalArgumentException("都道府県は3つまでです");u.getPrefectures().clear();u.getPrefectures().addAll(prefectures.findAllById(r.prefectureIds()));}return u; }
