import {mediaHref,mediaEmbed,mediaProvider,mediaFields,mediaIconRow} from './media-embed.js?v=20260919-3';
import {api,h,icon,avatar,state,main,showPage,notice,empty,button,toast,bindForm,confirmAction,report,choices,counter,requireUser,verificationNotice,openImageCropper,loginRelativeTime} from './ui.js?v=20260919-3';
...
async function profilePage(id){
  try {
    ...
    const mediaItems=[p.youtubeUrl,p.tiktokUrl,p.soundcloudUrl,p.spotifyUrl,p.appleMusicUrl].filter(url=>mediaHref(url));
    ...
    // URL" must not render as a trusted-looking Spotify link to somewhere else. Only YouTube gets
    // an embedded player here - the others are one tap away on their own app, and a page of five
    // stacked players was more scroll than payoff.
    const mediaMarkup=mediaItems.length?`<section class="detail-section profile-video"><h2>演奏動画・音源</h2><div class="profile-media-list">${mediaItems.map(url=>{const provider=mediaProvider(url);const label=provider||'他のリンク';const key=mediaFields.find(([,l])=>l===provider)?.[2];const embed=provider==='YouTube'?mediaEmbed(url):null;return `<article class="profile-media-item${embed?' has-embed':''}">${embed?`<iframe class="media-frame" style="${embed.ratio?`aspect-ratio:${embed.ratio}`:`height:${Number(embed.height)}px`}${embed.width?`;max-width:${Number(embed.width)}px`:''}" src="${h(embed.src)}" title="${h(p.username)}の${h(label)}" loading="lazy" allow="encrypted-media; fullscreen; clipboard-write" allowfullscreen></iframe>`:''}<a class="row media-link" href="${h(mediaHref(url))}" target="_blank" rel="noopener noreferrer">${key?icon(key):''}${h(label)}</a></article>`}).join('')}</div></section>`:'';
...
async function profileEdit(){
  ...
  bindForm(form,async fd=>{
    const body={username:fd.get('username').trim(),bio:fd.get('bio').trim(),age:fd.get('age')?Number(fd.get('age')):null,experienceYears:fd.get('experienceYears')?Number(fd.get('experienceYears')):null,gender:fd.get('gender')||null,youtubeUrl:fd.get('youtubeUrl').trim()||null,tiktokUrl:fd.get('tiktokUrl').trim()||null,soundcloudUrl:fd.get('soundcloudUrl').trim()||null,spotifyUrl:fd.get('spotifyUrl').trim()||null,appleMusicUrl:fd.get('appleMusicUrl').trim()||null};
...
function supportPage(){
  showPage(`<div class="page help-page">
    ...
    ${helpSection('help-start', '使いはじめる', [
      ['登録直後、確認画面から他の画面へ進めない', 'メールアドレスの確認が終わるまでは、募集の投稿やメッセージの送信はできません。募集の閲覧・検索は確認前でも利用できます。登録時のメール内リンクを開くと、すべての機能が使えるようになります。', ['メールアドレスを確認する', '/verify-email']],
    ...
    ${helpSection('help-posts', '募集のきまり', [
      ...
      ['投稿の「更新」が12時間に1回までしかできない', '本文・条件・画像の編集はいつでも自由に行えます。一覧の上位に再浮上させる「更新」操作だけ、12時間に1回までの制限があります。', null],
    ...
    ${helpSection('help-people', '相手とのやりとり', [
      ...
      ['不適切な募集や利用者を見つけた', '募集の詳細画面・プロフィール画面にある「通報」から運営へ知らせてください。メッセージについては今のところ通報ボタンがないため、お問い合わせからご連絡ください。', null],
    ...
    <section class="help-contact" aria-labelledby="help-contact-title">
      ...
      <p>募集・プロフィールの内容についての連絡は、各画面の「通報」から運営に届きます。メッセージについての連絡はお問い合わせからお送りください。</p>
