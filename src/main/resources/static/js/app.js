...
function header(){
 ...
 el.innerHTML=`<div class="header-inner ${state.user?'is-authed':'is-guest'}">${brand('a','href="/posts" aria-label="Band Link ホーム"')}<nav class="main-nav" aria-label="メインナビゲーション"><a href="/posts" class="${here==='/'||here==='/posts'?'active':''}">仲間を探す</a>${state.user?`<a href="/my/posts" class="${here==='/my/posts'?'active':''}">自分の投稿</a>`:''}${feedbackLinks}</nav><div class="header-actions">${state.user?`<a class="icon-button" data-notif-link href="/notifications" aria-label="通知">${icon('bell')}<span data-unread-dot class="dot" hidden></span></a><a class="icon-button" href="/messages" aria-label="メッセージ">${icon('message')}</a><a class="button secondary header-profile" href="/users/${state.user.id}">プロフィール</a>`:`${here==='/login'?'':'<a class="button secondary" href="/login">ログイン</a>'}${here==='/register'?'':'<a class="button primary register-cta" href="/register">新規登録</a>'}`}</div><a class="icon-button x-link" href="https://x.com/Band_Link_info" target="_blank" rel="noopener noreferrer" aria-label="X（旧Twitter）でBand Linkをフォロー">${icon('x')}</a></div>`;
