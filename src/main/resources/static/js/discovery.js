...
import {mediaIconRow} from './media-embed.js?v=20260918-1';
import {api,h,icon,avatar,state,main,showPage,notice,empty,button,toast,report,confirmAction,time,relativeTime,loginRelativeTime,ages,frequencyLabel,chips,requireUser,verificationNotice,openImageViewer} from './ui.js?v=20260918-2';
...
export function postCard(post) {
 ...
 return `<article class="post-card" data-post-id="${Number(post.id)}">
  <div class="post-thumb">${avatar(author,true)}</div><div class="post-content"><h2><span class="post-mark ${joining?'join':''}" aria-hidden="true" title="${joining?'加入希望':'募集'}">${joining?'加':'募'}</span><a href="/posts/${Number(post.id)}"><span class="sr-only">${joining?'加入希望':'募集'}：</span>${h(post.title)}</a></h2><p class="post-roles"><span>${joining?'加入':'募集'}</span>${h(labelNames(post.parts))}</p><p class="post-location">${h(labelNames(post.prefectures))}</p><div class="post-person"><a href="/users/${Number(post.userId)}" class="person"><span><span class="person-name">${h(post.username)}</span>${post.authorAgeRange?`<span class="person-age">${h(post.authorAgeRange)}</span>`:''}${post.authorGender?`<span class="person-gender">${h(post.authorGender)}</span>`:''}</span></a><span class="post-media-icons">${mediaIconRow(author,icon,h,true)}</span><span class="post-times">最終ログイン ${h(loginLabel)}${post.authorOnline?'<span class="online-dot" aria-hidden="true"></span>':''}　投稿 ${h(relativeTime(post.createdAt))}</span><div class="post-contact">${contact}<a class="read-post" href="/posts/${Number(post.id)}">詳細を読む </a></div></div></div>
 </article>`;
}
...
async function detail(id){
 ...
 </article><aside class="sidebar author-contact"><h2>投稿者</h2><a class="detail-person" href="/users/${p.userId}">${avatar(author,true)}<span><strong>${h(p.username)}</strong><small>最終ログイン ${h(loginRelativeTime(p.authorLastLoginAt,p.authorOnline))}</small></span></a>${author.bio?`<p class="author-excerpt">${h(author.bio)}</p>`:''}<a class="read-post" href="/users/${p.userId}">プロフィールを読む </a><div class="author-message">${own?button('自分の募集を管理','/my/posts'):button('メッセージを送る',state.user?'/messages?to='+p.userId:'/login?next='+encodeURIComponent('/messages?to='+p.userId))}${own?'<p>編集・終了・再公開は自分の募集から行えます。</p>':''}</div></aside></div></div>`,p.title,p.content);
