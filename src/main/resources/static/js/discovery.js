...
export function postCard(post) {
 ...
 return `<article class="post-card" data-post-id="${Number(post.id)}">
  ...
  <div class="post-person"><a href="/users/${Number(post.userId)}" class="person">${avatar({username:post.username,profileImageUrl:post.authorImageUrl})}<span><span class="person-name">${h(post.username)}</span>${post.authorAgeRange?`<span class="person-age">${h(post.authorAgeRange)}</span>`:''}${post.authorOnline?`<span class="person-online">オンライン中<span class="online-dot" aria-hidden="true"></span></span>`:''}${post.authorActivity&&!post.authorOnline?`<span class="person-sub">${h(post.authorActivity)}</span>`:''}</span></a><div class="post-contact">${contact}<a class="read-post" href="/posts/${Number(post.id)}">詳細を読む ${icon('arrow')}</a></div></div>
 </article>`;
}
