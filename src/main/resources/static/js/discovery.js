...
export function postCard(post) {
 ...
 return `<article class="post-card" data-post-id="${Number(post.id)}">
  <div class="post-thumb">${avatar(author,true)}</div><div class="post-content"><h2><span class="post-mark ${joining?'join':''}" aria-hidden="true" title="${joining?'加入希望':'募集'}">${joining?'加':'募'}</span><a href="/posts/${Number(post.id)}"><span class="sr-only">${joining?'加入希望':'募集'}：</span>${h(post.title)}</a></h2><p class="post-roles"><span>${joining?'加入':'募集'}</span>${h(labelNames(post.parts))}</p><p class="post-location">${h(labelNames(post.prefectures))}</p><div class="post-person"><a href="/users/${Number(post.userId)}" class="person"><span><span class="person-name">${h(post.username)}</span>${post.authorAgeRange?`<span class="person-age">${h(post.authorAgeRange)}</span>`:''}${post.authorGender?`<span class="person-gender">${h(post.authorGender)}</span>`:''}</span></a><span class="post-times">最終ログイン ${h(loginLabel)}　投稿 ${h(relativeTime(post.createdAt))}</span><div class="post-contact">${contact}<a class="read-post" href="/posts/${Number(post.id)}">詳細を読む ${icon('arrow')}</a></div></div></div>
 </article>`;
}
