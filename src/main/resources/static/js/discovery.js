...
import {mediaIconRow} from './media-embed.js';
import {api,h,icon,avatar,state,main,showPage,notice,empty,button,toast,report,confirmAction,time,relativeTime,loginRelativeTime,ages,frequencyLabel,chips,requireUser,verificationNotice,openImageViewer} from './ui.js?v=20260915-3';
...
export function postCard(post) {
 ...
 const author={username:post.username,profileImageUrl:post.authorImageUrl,youtubeUrl:post.authorYoutubeUrl,tiktokUrl:post.authorTiktokUrl,soundcloudUrl:post.authorSoundcloudUrl,spotifyUrl:post.authorSpotifyUrl,appleMusicUrl:post.authorAppleMusicUrl};
 return `<article class="post-card" data-post-id="${Number(post.id)}">
  <div class="post-thumb">${avatar(author,true)}</div><div class="post-content"><h2><span class="post-mark ${joining?'join':''}" aria-hidden="true" title="${joining?'加入希望':'募集'}">${joining?'加':'募'}</span><a href="/posts/${Number(post.id)}"><span class="sr-only">${joining?'加入希望':'募集'}：</span>${h(post.title)}</a></h2><p class="post-roles"><span>${joining?'加入':'募集'}</span>${h(labelNames(post.parts))}</p><p class="post-location">${h(labelNames(post.prefectures))}</p><div class="post-person"><a href="/users/${Number(post.userId)}" class="person"><span><span class="person-name">${h(post.username)}</span>${post.authorAgeRange?`<span class="person-age">${h(post.authorAgeRange)}</span>`:''}${post.authorGender?`<span class="person-gender">${h(post.authorGender)}</span>`:''}</span></a><span class="post-media-icons">${mediaIconRow(author,icon,h,true,false)}</span><span class="post-times">最終ログイン ${h(loginLabel)}${post.authorOnline?'<span class="online-dot" aria-hidden="true"></span>':''}　投稿 ${h(relativeTime(post.createdAt))}</span><div class="post-contact">${contact}<a class="read-post" href="/posts/${Number(post.id)}">詳細を読む </a></div></div></div>
 </article>`;
}
...
async function detail(id){
 ...
 showPage(`<div class="page post-detail-page"><a class="back-link" href="${h(backUrl)}">募集一覧へ</a><div class="detail-layout"><article class="detail-article">
 ...
 </article><aside class="sidebar author-contact"><h2>投稿者</h2><a class="detail-person" href="/users/${p.userId}">${avatar(author,true)}<span><strong>${h(p.username)}</strong><small>最終ログイン ${h(loginRelativeTime(p.authorLastLoginAt,p.authorOnline))}</small></span></a>${author.bio?`<p class="author-excerpt">${h(author.bio)}</p>`:''}${mediaIconRow(author,icon,h)}<a class="read-post" href="/users/${p.userId}">プロフィールを読む </a><div class="author-message">${own?button('自分の募集を管理','/my/posts'):button('メッセージを送る',state.user?'/messages?to='+p.userId:'/login?next='+encodeURIComponent('/messages?to='+p.userId))}${own?'<p>編集・終了・再公開は自分の募集から行えます。</p>':''}</div></aside></div></div>`,p.title,p.content);
...
async function ownPosts(){
 const posts=(await api('/api/posts/mine')).sort((a,b)=>(a.type==='WANTS_TO_JOIN')-(b.type==='WANTS_TO_JOIN'));showPage(`<div class="page own-posts-page"><div class="page-heading"><div><h1>自分の投稿</h1><p>公開中の募集と、これまでの投稿を管理します。</p></div>${button('新しい投稿を作成','/posts/new')}</div><div class="stack">${verificationNotice()}${posts.length?posts.map(p=>{const joining=p.type==='WANTS_TO_JOIN';return `<article class="panel"><div class="row spread"><span class="badge ${p.status==='OPEN'?'badge-open':'badge-closed'}">${p.status==='OPEN'?'公開中':'募集終了'}</span><span class="hint">掲載期限 ${h(time(p.expiresAt))}</span></div><h2 style="margin:20px 0;font-size:18px;display:flex;align-items:flex-start;gap:8px"><span class="post-mark ${joining?'join':''}" aria-hidden="true" title="${joining?'加入希望':'募集'}">${joining?'加':'募'}</span><span style="margin-top:7px">${h(p.title)}</span></h2><div class="row">${button('詳細を見る','/posts/'+p.id,'quiet')}${p.status==='OPEN'?`${button('編集する','/posts/'+p.id+'/edit','quiet')}<button class="button quiet" data-boost="${p.id}">更新する</button>`:['MANUAL','EXPIRED'].includes(p.closedReason)?`<button class="button primary" data-reopen="${p.id}">再公開する</button>`:'<span class="hint">この投稿は再公開できません。</span>'}</div>${p.status==='OPEN'?`<div class="row" style="margin-top:8px"><button class="button quiet" data-close="${p.id}">投稿を終了</button></div>`:''}</article>`;}).join(''):empty('投稿を作成してみましょう','募集または加入希望を投稿すると、音楽仲間が見つかりやすくなります。',button('投稿を作成する','/posts/new'))}</div><p class="hint" style="margin-top:24px">募集と加入は、それぞれ1件ずつ公開できます。新規投稿・編集に制限はありません。「更新する」で一覧の上位に上がりますが、12時間に1回までです。</p></div>`,'自分の投稿');
 ...
 // Align 投稿を終了 under 編集する rather than guessing a fixed offset, since the buttons above it
 ...
 main.querySelectorAll('[data-close]').forEach(el=>{
  ...
  if(!editBtn)return;
  const delta=editBtn.getBoundingClientRect().left-7-el.getBoundingClientRect().left;
