import {api,h,icon,avatar,state,main,showPage,notice,empty,button,toast,bindForm,report,confirmAction,time,relativeTime,navigate,frequencies,ages,frequencyLabel,chips,choices,counter,requireUser,verificationNotice} from './ui.js';
...
export function postCard(post) {
  ...
  const own = state.user && String(state.user.id) === String(post.userId);
  // Contacting someone used to mean list -> detail -> sidebar. The person is the point, so the
  // action sits on the card next to them.
  const contact = state.user && !own
    ? `<a class="button small" href="/messages?to=${Number(post.userId)}">メッセージ</a>`
    : `<a class="button small quiet" href="/posts/${Number(post.id)}">詳しく見る</a>`;
  return `<article class="post-card" data-post-id="${Number(post.id)}">
    <div class="card-top"><span class="post-type ${joining?'join':''}">${joining?'参加希望':'メンバー募集'}</span><span class="post-age">${h(relativeTime(post.createdAt))}</span></div>
    ...
    ${chips(post.genres)}
    <div class="card-bottom"><a href="/users/${Number(post.userId)}" class="person">${avatar({username:post.username})}<div><div class="person-name">${h(post.username)}</div><div class="person-sub">${post.authorActivity?h(post.authorActivity):h(frequencyLabel(post.activityFrequency))}</div></div></a>${contact}</div>
  </article>`;
}
...
async function listing(){
 ...
 const append=async()=>{
   ...
   try {
     ...
     // The intro claims the board is worth browsing, so it states the live number rather than a mood.
     const live=main.querySelector('[data-live-count]');
     if(live)live.textContent=hasNext?`いま${total}件以上の募集が出ています。`:`いま${total}件の募集が出ています。`;
