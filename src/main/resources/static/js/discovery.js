...
async function detail(id){
 ...
 </article><aside class="sidebar author-contact"><h2>投稿者</h2><a class="detail-person" href="/users/${p.userId}">${avatar(author,true)}<span><strong>${h(p.username)}</strong><small>最終ログイン ${h(loginRelativeTime(p.authorLastLoginAt,p.authorOnline))}</small></span></a>${author.bio?`<p class="author-excerpt">${h(author.bio)}</p>`:''}${mediaIconRow(author,icon,h)}<a class="read-post" href="/users/${p.userId}">プロフィールを読む ${icon('arrow')}</a><div class="author-message">${own?button('自分の募集を管理','/my/posts'):button('メッセージを送る',state.user?'/messages?to='+p.userId:'/login?next='+encodeURIComponent('/messages?to='+p.userId))}${own?'<p>編集・終了・再公開は自分の募集から行えます。</p>':''}</div></aside></div></div>`,p.title);
