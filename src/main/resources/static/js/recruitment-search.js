 ...
 showPage(`<div class="page discovery-page">
  ...
  <div class="board-layout">
   <aside class="search-rail" aria-label="募集の検索"><form id="search-form">
    <div class="search-keyword" id="search-keyword"><label for="keyword">キーワード</label><div class="search-bar"><input id="keyword" class="search-input" name="keyword" maxlength="100" placeholder="アーティスト・駅名など" value="${h(params.get('keyword'))}"><button type="submit" class="button primary" aria-label="キーワードで検索">${icon('search')}</button></div></div>
    <div class="search-actions"><button class="button secondary search-toggle" type="button" aria-controls="search-keyword" aria-expanded="false">${icon('search')}<span>キーワード</span></button><button class="button secondary filter-toggle" type="button" aria-controls="filters" aria-expanded="${filterOpen}">${icon('filter')}条件を絞る <span>${applied.length?applied.length+'項目選択中':''}</span></button></div>
